...
const uint CAPTURE_PIN_BASE = 22; //CHANGED PIN from 16 to 22 to match stemma
const uint CAPTURE_PIN_COUNT = 2; 
const uint CAPTURE_N_SAMPLES = 96; //CHANGE TO INCREASE length of sample 96 by default
...
int main() {
    stdio_init_all();
    while(stdio_usb_connected()!=true); //NEW LINE ADDED BC OF ED STEM
    gpio_init(21); //from flashlight
    gpio_set_dir(21, GPIO_IN); //from flashlight
    ...
    printf("Push the BOOT button to begin");
